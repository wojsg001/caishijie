...
@interface SJInteractionView ()<UITableViewDataSource,UITableViewDelegate,UITextFieldDelegate,TLChatBoxFaceViewDelegate,TLChatBoxMoreViewDelegate,UIScrollViewDelegate,SJSpeechRecognizerViewDelegate,IFlySpeechRecognizerDelegate>
{
    // 获取键盘弹出的动画时间
    CGFloat durtion;
...
@implementation SJInteractionView
...
- (void)awakeFromNib
{
    ...
    UITapGestureRecognizer *tapGesture = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(cancelFirstResponder)];
...
- (void)sendMessage:(NSString *)text
{
    ...
    // 发送互动
    [netManager sendInteractionWithToken:md5Auth_key andUserid:user_id andTime:datestr andTargetid:self.liveUserDict[@"user_id"] andContent:text andReplyid:self.replyid success:^(NSDictionary *dict) {
...
- (IBAction)sendBtnPressed:(id)sender
{
    ...
    {
        if (self.delegate && [self.delegate respondsToSelector:@selector(showLoginViewFromInteractionView)])
        {
            [self.delegate showLoginViewFromInteractionView];
        }
...
- (void)selectBtn:(UIButton *)btn
{
    ...
    self.textField.placeholder = [NSString stringWithFormat:@"回复:%@",messageF.message.interactM.nickname];
...
{
    if (self.delegate && [self.delegate respondsToSelector:@selector(interactionSelectImageWithType:andReplyid:)])
    {
        // 相片
        if (itemType == TLChatBoxItemAlbum)
        {
            [self.delegate interactionSelectImageWithType:TLChatBoxItemAlbum andReplyid:self.replyid];
        }
        // 拍摄
        else if (itemType == TLChatBoxItemCamera)
        {
            [self.delegate interactionSelectImageWithType:TLChatBoxItemCamera andReplyid:self.replyid];
        }
    }
