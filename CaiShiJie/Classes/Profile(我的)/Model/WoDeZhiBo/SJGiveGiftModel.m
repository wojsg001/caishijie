...
@implementation SJGiveGiftModel
...
- (void)setData:(NSString *)data
{
    ...
    _senderModel = [SJSenderModel objectWithKeyValues:dict];
}
...
- (NSString *)created_at
{
    ...
    if ([created_at isThisYear]) { // 今年
        ...
        if ([created_at isToday]) { // 今天
            ...
            if (cmp.hour >= 1) {
                return [NSString stringWithFormat:@"%li小时之前",(long)cmp.hour];
            }else if (cmp.minute > 1){
                return [NSString stringWithFormat:@"%li分钟之前",(long)cmp.minute];
            }else{
                return @"刚刚";
            }
