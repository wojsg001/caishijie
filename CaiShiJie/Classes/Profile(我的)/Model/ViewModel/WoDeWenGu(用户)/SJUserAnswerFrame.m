...
@implementation SJUserAnswerFrame

- (void)setAnswerModel:(SJUserAnswerModel *)answerModel
{
...
- (void)parseAllKeywords {
    NSString *tmpStr = self.answerModel.answerDetail.content;
    if (tmpStr.length > 0) {
...
- (void)calculateHegihtAndAttributedString {
    NSString *str = [SJParser getHandleString:self.answerModel.answerDetail.content];
...
- (void)calculateCellHegiht {
    ...
    CGFloat nameY = 10;
    CGSize nameSize = [_answerModel.answerDetail.nickname sizeWithAttributes:@{NSFontAttributeName:[UIFont systemFontOfSize:15]}];
    _nameF = (CGRect){{nameX, nameY}, nameSize};
    
    // 时间
    CGFloat timeY = 10;
    CGSize timeSize = [_answerModel.answerDetail.created_at sizeWithAttributes:@{NSFontAttributeName:[UIFont systemFontOfSize:12]}];
    ...
    CGSize levelSize = [_answerModel.answerDetail.honor sizeWithAttributes:@{NSFontAttributeName:[UIFont systemFontOfSize:12]}];
    _levelF = (CGRect){{levelX, levelY}, levelSize};
    
    // 回答
    CGFloat answerX = 10;
    CGFloat answerY = CGRectGetMaxY(_levelF) + 10;
    _answerF = (CGRect){{answerX, answerY}, {SJScreenW - 20, _contentHeight}};
    
    // 线条一
    CGFloat line1X = 0;
    CGFloat line1Y = CGRectGetMaxY(_answerF) + 10;
    CGFloat line1W = SJScreenW;
    CGFloat line1H = 0.5f;
    _line1ViewF = CGRectMake(line1X, line1Y, line1W, line1H);
    
    // 计算cell的高度
    _cellH = CGRectGetMaxY(_line1ViewF);
}
