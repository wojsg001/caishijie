...
@interface SJZhuaNiuGuViewController ()<UITableViewDelegate,UITableViewDataSource>
{
    int i; // 分页
}
...
@implementation SJZhuaNiuGuViewController
...
- (void)viewDidLoad {
    ...
    [self loadZhuaNiuGuData];
    
    // 添加下拉刷新
    [self.tableView addHeaderWithTarget:self action:@selector(loadZhuaNiuGuData)];
    // 添加下拉获取更多数据
    [self.tableView addFooterWithTarget:self action:@selector(loadMoreZhuaNiuGuData)];
...
- (void)loadZhuaNiuGuData
{
    i = 1;
    NSString *urlStr = [NSString stringWithFormat:@"%@/mobile/article/findbytype",HOST];
    NSDictionary *dic = @{@"pageindex":[NSString stringWithFormat:@"%i",i],@"pagesize":@"10",@"type":@"2"};
