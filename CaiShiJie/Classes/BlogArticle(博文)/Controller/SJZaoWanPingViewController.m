...
@interface SJZaoWanPingViewController ()<UITableViewDelegate,UITableViewDataSource>
{
    int i; // 分页
}
...
@implementation SJZaoWanPingViewController
...
- (void)viewDidLoad {
    ...
    [self loadZaoWanPingData];
    
    // 添加下拉刷新
    [self.tableView addHeaderWithTarget:self action:@selector(loadZaoWanPingData)];
    // 添加下拉获取更多数据
    [self.tableView addFooterWithTarget:self action:@selector(loadMoreZaoWanPingData)];
...
- (void)loadZaoWanPingData
{
    i = 1;
    NSString *urlStr = [NSString stringWithFormat:@"%@/mobile/article/findbytype",HOST];
    NSDictionary *dic = @{@"pageindex":[NSString stringWithFormat:@"%i",i],@"pagesize":@"10",@"type":@"1"};
