...
@interface SJStockDayLineViewController ()
...
@implementation SJStockDayLineViewController
...
- (void)viewDidLoad {
    ...
    [self loadDayLineData];
}

- (void)loadDayLineData {
    NSTimeInterval timeInterval = [[NSDate date] timeIntervalSince1970];
    NSString *urlStr = [NSString stringWithFormat:@"https://gupiao.baidu.com/api/stocks/stockdaybar?from=pc&os_ver=1&cuid=xxx&vv=100&format=json&stock_code=%@&step=3&start=&count=160&fq_type=no&timestamp=%ld", self.stock_code, (long)timeInterval];
    //NSLog(@"%@", urlStr);
