//
//  SJChatRoomPopularityListViewController.m
//  CaiShiJie
//
...
@interface SJChatRoomPopularityListViewController ()<UITableViewDelegate,UITableViewDataSource>
{
    int i;
}
...
@implementation SJChatRoomPopularityListViewController
...
- (void)viewDidLoad {
    ...
    // 加载今日热门数据
    [self loadTodayHotLiveData];
    
    [self.tableView addHeaderWithTarget:self action:@selector(loadTodayHotLiveData)];
    [self.tableView addFooterWithTarget:self action:@selector(loadMoreTodayHotLiveData)];
...
- (void)setUpTableView
{
    ...
    UINib *nib = [UINib nibWithNibName:@"SJChatRoomPopularityCell" bundle:nil];
    [self.tableView registerNib:nib forCellReuseIdentifier:@"SJChatRoomPopularityCell"];
}

#pragma mark - 加载新数据
- (void)loadTodayHotLiveData
{
    i = 1;
    
    NSString *urlStr = [NSString stringWithFormat:@"%@/mobile/ranking/live",HOST];
    NSDictionary *dic = @{@"type":@"1",@"pageindex":[NSString stringWithFormat:@"%i",i],@"pagesize":@"10"};
...
{
    SJChatRoomPopularityCell *cell = [tableView dequeueReusableCellWithIdentifier:@"SJChatRoomPopularityCell"];
