...
@interface ATJWebViewController ()<WKNavigationDelegate,WKUIDelegate,WKScriptMessageHandler,UINavigationControllerDelegate,UINavigationBarDelegate,ATJURLFailtureDelegate,UIGestureRecognizerDelegate>

@property (nonatomic, strong) ATJURLFailture * netWorkingFailtureView;///加载失败
...
@implementation ATJWebViewController
...
- (void)webViewloadURLType{
    switch (self.loadType) {
        case loadWebURLString:{
            ...
            self.netWorkingFailtureView.hidden = YES;
            break;
        }
...
//        self.customBackBarItem.customView.hidden = NO;
...
//        
//        self.customBackBarItem.customView.hidden = YES;
//    }
}
//请求链接处理
...
-(void)webView:(WKWebView *)webView didFailProvisionalNavigation:(WKNavigation *)navigation withError:(NSError *)error{
    ...
    self.netWorkingFailtureView.hidden = NO;
}
...
- (WKWebView *)wkWebView{
    if (!_wkWebView) {
        ...
        _wkWebView = [[WKWebView alloc] initWithFrame:CGRectMake(0, 0, self.view.frame.size.width, self.view.frame.size.height - NAVIGATIONBAR_HEIGHT) configuration:Configuration];
...
-(UIBarButtonItem*)customBackBarItem{
    if (!_customBackBarItem) {
        UIImage* backItemImage = [[UIImage imageNamed:@"back"] imageWithRenderingMode:UIImageRenderingModeAutomatic];
        UIImage* backItemHlImage = [[UIImage imageNamed:@"back"] imageWithRenderingMode:UIImageRenderingModeAutomatic];
