...
- (void) setFrame:(CGRect)frame
{
    ...
    [self.scrollView setFrame:CGRectMake(0, 0, frame.size.width, frame.size.height - 18)];
...
{
    int faceIconCount = 20;
    if (WIDTH_SCREEN > HEIGHT_SCREEN) {
        // 横屏
        faceIconCount = 38;
    }
    int page = (int)(self.curGroup.facesArray.count / (self.curGroup.faceType == TLFaceTypeEmoji ? faceIconCount : 8)) + (((int)(self.curGroup.facesArray.count % (self.curGroup.faceType == TLFaceTypeEmoji ? faceIconCount : 8))) == 0 ? 0 : 1);
...
- (void) showFaceFageAtIndex:(NSUInteger)index
{
    ...
    int count = _curGroup.faceType == TLFaceTypeEmoji ? faceIconCount : 8;
    if (_curPage == -1) {
...
- (TLChatBoxFaceMenuView *) faceMenuView
{
    if (_faceMenuView == nil) {
        _faceMenuView = [[TLChatBoxFaceMenuView alloc] initWithFrame:CGRectMake(0, self.frameHeight - HEIGHT_BOTTOM_VIEW, WIDTH_SCREEN, 0)];
