...
- (void) showFaceGroup:(TLFaceGroup *)group formIndex:(int)fromIndex count:(int)count
{
    ...
    int col = (group.faceType == TLFaceTypeEmoji ? rowIconCount : 4);
    ...
    for (int i = fromIndex; i < fromIndex + count; i ++) {
        ...
        if (i >= group.facesArray.count || i < 0) {
            ...
            button.tag = i;
            [button setImage:[UIImage imageNamed:[NSString stringWithFormat:@"MLEmoji_Expression.bundle/%@", face.faceID]] forState:UIControlStateNormal];
