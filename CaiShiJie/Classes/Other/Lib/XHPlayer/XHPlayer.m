...
//    }
    ...
    if (self.isFullScreenBlock) {
        self.isFullScreenBlock(FullScreenButton.selected);
    }
        ...
        if ([keyPath isEqualToString:XHPlayerEmptyBufferKey]) {
            ...
            switch (status) {
                case AVPlayerStatusReadyToPlay: { // 准备好播放
                    [self.player pause];
                    [self hideIndicator];
                    self.playerView.bottomView.controlButton.selected = NO;
