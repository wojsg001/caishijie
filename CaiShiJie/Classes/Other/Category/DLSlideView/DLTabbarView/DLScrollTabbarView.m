...
- (void)switchingFrom:(NSInteger)fromIndex to:(NSInteger)toIndex percent:(float)percent{
    ...
    CGFloat width = toWidth * percent + fromWidth*(1-percent) + 10;
    CGFloat x = fromX + (toX - fromX)*percent - 5;
...
- (void)setSelectedIndex:(NSInteger)selectedIndex{
    if (_selectedIndex != selectedIndex) {
        ...
        if (selectedIndex >= 0 && selectedIndex < [self tabbarCount]) {
            ...
            trackView_.frame = CGRectMake(trackRc.origin.x-5, trackView_.frame.origin.y, trackRc.size.width+10, CGRectGetHeight(trackView_.bounds));
        }
