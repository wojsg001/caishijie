...
- (void)layoutTabbar{
    ...
    float trackX = width*self.selectedIndex + 20;
    trackView_.frame = CGRectMake(trackX, trackView_.frame.origin.y, width - 40, kTrackViewHeight);
}
...
- (void)switchingFrom:(NSInteger)fromIndex to:(NSInteger)toIndex percent:(float)percent{
    ...
    trackView_.frame = CGRectMake(trackX + 20, trackView_.frame.origin.y, CGRectGetWidth(trackView_.bounds), CGRectGetHeight(trackView_.bounds));
}
...
- (void)setSelectedIndex:(NSInteger)selectedIndex{
    if (_selectedIndex != selectedIndex) {
        ...
        float width = self.bounds.size.width/self.tabbarItems.count;
        float trackX = width*selectedIndex + 20;
