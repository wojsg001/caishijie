//
//  SJMostInteractionViewController.m
//  CaiShiJie
//
...
@interface SJMostInteractionViewController ()<UITableViewDelegate,UITableViewDataSource>
{
    int i;
}
...
@implementation SJMostInteractionViewController
...
- (void)viewDidLoad {
    ...
    // 加载今日热门数据
    [self loadMostInteractionLiveData];
    
    [self.tableView addHeaderWithTarget:self action:@selector(loadMostInteractionLiveData)];
    [self.tableView addFooterWithTarget:self action:@selector(loadMoreMostInteractionLiveData)];
...
#pragma mark - 加载新数据
- (void)loadMostInteractionLiveData
{
    i = 1;
    
    NSString *urlStr = [NSString stringWithFormat:@"%@/mobile/ranking/live",HOST];
    NSDictionary *dic = @{@"type":@"3",@"pageindex":[NSString stringWithFormat:@"%i",i],@"pagesize":@"10"};
...
#pragma mark - 加载更多数据
- (void)loadMoreMostInteractionLiveData
{
    i = i + 1;
